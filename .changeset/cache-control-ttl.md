...
- **`'default'` in model overrides** — per-model `cacheControlTtl` supports `'default'` to cancel a global TTL and revert to Anthropic's default behavior for specific models.
