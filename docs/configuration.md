...
A model name matches with or without its vendor prefix: the bare `kimi-k2.6` matches the override key `moonshotai/kimi-k2.6`. A dated or variant slug needs an explicit `*` — `moonshotai/kimi-k2.6-20260420` matches `moonshotai/kimi-k2.6*`, not the bare `moonshotai/kimi-k2.6` key — so `gpt-4` never captures `gpt-4o`. The incoming model name is forwarded upstream unchanged.

> **Same-slug collisions:** if several override keys share a model name across vendors (e.g. `openai/gpt-4o` and `azure/gpt-4o`) and Claude Code sends the bare name, the **first-declared** key wins. `proxitor` warns once at startup and in `proxitor doctor`; send the vendor-prefixed name to pick a specific one.
