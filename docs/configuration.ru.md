...
| **Стабилизировать префикс** | `normalizeVolatileSystem` | `false` | Вырезает волатильный хэш `cch` из Claude Code из системного промпта, чтобы префикс перестал меняться (не-Anthropic провайдеры) |
...
- **Не-Anthropic модели за Claude Code (qwen / glm / …)** — рычаг 3 (`normalizeVolatileSystem`) стабилизирует префикс; без него меняющийся хэш `cch` не даёт префиксному кэшу согреться.
...
Claude Code встраивает в системный промпт волатильный хэш `cch=…`, который меняется (почти) на каждом ходу. Для **нативных для Anthropic** провайдеров это безвредно — ключ кэша его поглощает. Но для **не-Anthropic** провайдеров (qwen, glm и других, маршрутизируемых через OpenRouter), эти меняющиеся байты лежат внутри кэшируемого префикса, поэтому префиксный кэш никогда не согревается и каждый ход платит полную цену.

`normalizeVolatileSystem` вырезает этот волатильный хэш из `messages[0]` (системного блока), чтобы байты префикса оставались стабильными от хода к ходу.

```yaml
normalizeVolatileSystem: true   # вырезать волатильный хэш cch из системного промпта
```
