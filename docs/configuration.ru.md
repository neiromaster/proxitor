...
| `openrouter-routing` | [Опции маршрутизации OpenRouter](#openrouter-routing-plugin) |
...
Липкая маршрутизация через заголовок `x-session-id`. Выводит стабильный ID сессии из логической модели, системного промпта и первого сообщения пользователя.
...
| Режим | Поведение |
| --- | --- |
| `auto` | Генерировать session ID если клиент не отправил |
